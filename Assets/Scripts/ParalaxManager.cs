...
public class ParalaxManager
{
    ...
    public void Update()
    {
        _backTransform.position = _backStartPosition + (_camera.transform.position - _cameraStartPositiom) * _backCoef;
        _middleTransform.position = _middleStartPosition + (_camera.transform.position - _cameraStartPositiom) * _middleCoef;
    }
}
